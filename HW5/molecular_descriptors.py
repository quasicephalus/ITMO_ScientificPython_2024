from descriptors import topology
from descriptors import constitution
from descriptors import burden
from descriptors import basak
from descriptors import cats2d
from descriptors import charge
from descriptors import connectivity
from descriptors import estate
from descriptors import geary
from descriptors import kappa
from descriptors import moe
from descriptors import moran
from descriptors import moreaubroto
...
def getAllDescriptors(data, mode='train', log=None):
    if mode == 'train':
        smiles, target = descriptor_target_split(data)
    else:
        smiles = data
    ...
    for i in range(len(smiles)):
        if log:
            log(f"Row {i+1} out of {len(smiles)}")
        print('Row %d out of %d' % (i + 1, len(smiles)), end='')
        print('\r', end='')
        AllDescriptors.loc[i] = getAllDescriptorsforMol(Chem.MolFromSmiles(smiles['SMILES'][i]))

    if mode == 'train':    
        final_df = descriptor_target_join(AllDescriptors, target)
    else:
        AllDescriptors['SMILES'] = smiles['SMILES']
        final_df = AllDescriptors

    print('Calculating Molecular Descriptors Completed.')
    return final_df